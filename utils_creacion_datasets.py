...
# La doble s es correcta, mirar aquí https://stanfordnlp.github.io/stanza/tokenize.html
# Necesito ponerlo porque si no al encontrar un punto lo separará en dos oraciones
...
def create_dependency_tags(dataset,encoding_type:list,separator:str):
    ...
    for doc in tqdm(docs_out):
        ...
        dataset[doc.text]['tokenized'] = ' '.join([token.text for token in doc.sentences[0].tokens])
